    ...
    <div class="workflow-layout" :class="{ 'workflow-layout--solo': teacher || !teacher }">
      ...
      <main v-if="run" class="wf-main" :class="{ 'wf-main--teacher': teacher, 'wf-main--student': !teacher }">
        <div v-if="teacher" class="wf-teacher-head">
          <div class="wf-teacher-head-main"><p class="eyebrow">当前实验记录</p><h2>{{ run.expName }}</h2><span class="wf-teacher-meta">{{ run.groupName }} · 第 {{ run.attempt }} 次</span><span class="wf-teacher-members">{{ run.members.map(m => m.name).join('、') }}</span></div>
          <label v-if="runs.length > 1" class="wf-run-picker"><span>切换实验记录（{{ runs.length }}）</span><select class="wf-run-select" :value="run.id" :disabled="busy" aria-label="选择实验记录" @change="onPickRun"><optgroup v-for="group in teacherRunGroups" :key="group.name" :label="group.name"><option v-for="item in group.items" :key="item.id" :value="item.id">{{ item.groupName }} · 第 {{ item.attempt }} 次 · {{ item.members.map(member => member.name).join('、') }}</option></optgroup></select></label>
        </div>
...
@media(min-width:901px){.workflow-layout--solo{max-width:1180px}.wf-teacher-toolbar{max-width:1180px}.wf-main--teacher{display:grid;grid-template-columns:238px minmax(0,1fr);align-items:stretch}.wf-main--teacher .wf-teacher-head{grid-column:1;grid-row:1;display:flex;flex-direction:column;padding:22px 18px;border-radius:14px 0 0 0;border-bottom:0}.wf-main--teacher .wf-teacher-tags{margin-top:auto}.wf-main--teacher .wf-stepper--teacher{grid-column:1;grid-row:2;display:flex;flex-direction:column;align-items:stretch;gap:6px;padding:10px;border:1px solid #e1e9ed;border-top:0;border-radius:0 0 0 14px}.wf-main--teacher .wf-stepper--teacher .wf-step{flex:0 0 auto;width:100%;justify-content:flex-start;padding:11px 12px}.wf-main--teacher>.wf-panel,.wf-main--teacher>.advanced-panel{grid-column:2;grid-row:1 / span 2;margin:0;border-radius:0 14px 14px 0;border-top:1px solid #e1e9ed;border-left:0}.wf-main--teacher>.wf-panel--deduct{display:grid;grid-template-columns:minmax(0,1.05fr) minmax(260px,.95fr);gap:26px}.wf-deduction-records{border-left:1px solid #e5edef;padding-left:26px}.wf-deduction-records .wf-empty{margin-top:12px}}
@media(max-width:900px){.wf-main--teacher>.wf-panel--deduct{display:block}.wf-deduction-records{margin-top:28px;padding-top:24px;border-top:1px solid #e5edef}}
</style>
